...
if(resets!==1||baseSpawns!==1||c.sel!==0||c.E.length!==1||c.SN[0]!=='FIRST FLIGHT'||c.puT!==99||!msg.includes('1/5'))throw Error('Easy did not enter a clean First Flight');
let e=c.E[0];c.snap=0;c.killE(e);if(c.sel||c.E.length!==1||c.E[0].hp!==1||!msg.includes('CHARGE FIRST'))throw Error('uncharged poke incorrectly counted as a tutorial Snap');
...
if(c.sel!==5||new Set(seen).size!==5||c.E.length||c.nextWave!==1.2||!msg.includes('FIRST FLIGHT COMPLETE'))throw Error('five successful Snaps did not complete First Flight cleanly');
c.D=1;c.wave=1;c.sel=0;c.spawnWave();if(c.SN[0]!=='THE DEAD FIELD'||baseSpawns!==2)throw Error('Normal Trial 1 was changed by Easy tutorial');c.killE({});if(baseKills!==1)throw Error('non-tutorial kills lost authoritative path');
