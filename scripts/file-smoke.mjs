...
try{
 ...
 const canvas=page.locator('#c');await canvas.waitFor({state:'visible',timeout:5000});await page.waitForTimeout(120);const box=await canvas.evaluate(n=>({w:n.getBoundingClientRect().width,h:n.getBoundingClientRect().height})),pos=(x,y)=>({x:box.w*x/960,y:box.h*y/640}),snap=()=>canvas.evaluate(n=>n.toDataURL()),label=()=>page.evaluate(()=>window.__pointerLabel||'');
 await page.evaluate(()=>{window.__controlsSeen=0;window.__pointerLabel=''});await page.keyboard.press('c');await page.waitForTimeout(70);if(!await page.evaluate(()=>!!window.__controlsSeen)||!/ON$/.test(await label()))throw Error('local Controls did not render pointer ON');await page.keyboard.press('Enter');await page.waitForTimeout(70);if(!/OFF$/.test(await label()))throw Error('local keyboard toggle did not render pointer OFF');await page.keyboard.press('m');await page.waitForTimeout(70);await page.evaluate(()=>{window.__controlsSeen=0;window.__pointerLabel=''});await canvas.click({position:pos(550,535)});await page.waitForTimeout(70);if(!await page.evaluate(()=>!!window.__controlsSeen)||!/OFF$/.test(await label()))throw Error('local Controls click failed while pointer gameplay OFF');await canvas.click({position:pos(600,300)});await page.waitForTimeout(70);if(!/ON$/.test(await label()))throw Error('local Canvas toggle did not render pointer ON');await page.keyboard.press('m');await page.waitForTimeout(70);
 ...
 console.log(`PASS: ${engine} opened ${artifact} via file://, rendered pointer Controls OFF/ON, Space reached gameplay, and PAUSED rendered`);await context.close();
