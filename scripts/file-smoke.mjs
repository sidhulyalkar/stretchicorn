...
try{
 ...
 const menu=await canvas.evaluate(n=>n.toDataURL());await page.keyboard.press('2');await page.waitForTimeout(180);const play=await canvas.evaluate(n=>n.toDataURL());
 ...
 console.log(`PASS: ${engine} opened ${artifact} directly via file://, rendered the title, started gameplay, and attempted no network requests`);await context.close();
