...
try{
 ...
 const canvas=page.locator('#c');await canvas.waitFor({state:'visible',timeout:5000});await page.waitForTimeout(120);const box=await canvas.evaluate(n=>({w:n.getBoundingClientRect().width,h:n.getBoundingClientRect().height})),pos=(x,y)=>({x:box.w*x/960,y:box.h*y/640}),snap=()=>canvas.evaluate(n=>n.toDataURL());
 await page.evaluate(()=>window.__controlsSeen=0);await page.keyboard.press('c');await page.waitForTimeout(70);if(!await page.evaluate(()=>!!window.__controlsSeen))throw Error('local Controls did not open');await page.keyboard.press('Enter');await page.keyboard.press('m');await page.waitForTimeout(70);if(await canvas.evaluate(n=>n.style.cursor)!=='default')throw Error('local pointer OFF cursor mismatch');await canvas.click({position:pos(550,535)});await page.waitForTimeout(60);await canvas.click({position:pos(600,300)});await page.keyboard.press('m');await page.waitForTimeout(70);if(await canvas.evaluate(n=>n.style.cursor)!=='crosshair')throw Error('local pointer re-enable failed');
 ...
 console.log(`PASS: ${engine} opened ${artifact} via file://, pointer Controls toggled OFF/ON, Space reached gameplay, and PAUSED rendered`);await context.close();
