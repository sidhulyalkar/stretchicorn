...
for(const q of['|STYLE|Higher difficulty = more Style.|','Aggression + close grazes/parries = more Style.'])if(!guide.includes(q))throw Error('missing Style explanation: '+q);for(const q of['STYLE SCORE','RUN RESULT','Beat corn. Style = mastery.','Best by difficulty. Time + ♥ show pace.'])if(guide.includes(q))throw Error('retired result copy survived in Field Guide: '+q);
