...
for(const q of['STYLE SCORE','Higher difficulty = higher Style.','Attack aggressively; chain hits to 4×.','Close grazes + parries reward risky dodges.'])if(!guide.includes(q))throw Error('missing Style explanation: '+q);for(const q of['RUN RESULT','Beat corn. Style = mastery.','Best by difficulty. Time + ♥ show pace.'])if(guide.includes(q))throw Error('retired result copy survived in Field Guide: '+q);
